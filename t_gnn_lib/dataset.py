...
def seq_to_graph_v2(seq_,seq_rel,norm_lap_matr = True):
    seq_ = seq_.squeeze()
    seq_rel = seq_rel.squeeze()
...
def decentralization_v2(seg_list_rel_temp, obs_len):
    ''' Implementation of equation 2 in the paper. '''
    # seg_list_rel_temp -> shape: (num_seq, 2, seg_len)
...
class TrajectoryDataset_Social(Dataset):
    """Dataloder for the Trajectory datasets"""
    def __init__(
        self, data_dir, obs_len=8, pred_len=8, skip=1, threshold=0.002,
        ...
        - delim: Delimiter in the dataset files
        """
        super(TrajectoryDataset_Social, self).__init__()
        ...
        # rel_coords = decentralization_v2(seq_list, self.obs_len)
        # self.obs_traj_rel = torch.from_numpy(rel_coords[:,:,:self.obs_len]).type(torch.float)
        # self.pred_traj_rel = torch.from_numpy(rel_coords[:,:,self.obs_len:]).type(torch.float)
        ...
        for ss in range(len(self.seq_start_end)):
            ...
            v_,a_ = seq_to_graph_v2(self.obs_traj[start:end,:],self.obs_traj_rel[start:end, :],self.norm_lap_matr)
            self.v_obs.append(v_.clone())
            self.A_obs.append(a_.clone())
            v_,a_=seq_to_graph_v2(self.pred_traj[start:end,:],self.pred_traj_rel[start:end, :],self.norm_lap_matr)
            self.v_pred.append(v_.clone())
            self.A_pred.append(a_.clone())
        pbar.close()
